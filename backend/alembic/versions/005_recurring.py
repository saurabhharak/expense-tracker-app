...
def upgrade() -> None:
    ...
    op.execute("CREATE INDEX idx_recurring_user ON expense_tracker.recurring_transactions (user_id)")
    op.execute("""
    CREATE INDEX idx_recurring_next_due ON expense_tracker.recurring_transactions (next_due_date)
        WHERE is_active = true
    """)

    op.execute("ALTER TABLE expense_tracker.recurring_transactions ENABLE ROW LEVEL SECURITY")
    op.execute("ALTER TABLE expense_tracker.recurring_transactions FORCE ROW LEVEL SECURITY")
...
def downgrade() -> None:
    op.execute("ALTER TABLE expense_tracker.transactions DROP CONSTRAINT IF EXISTS fk_txn_recurring")
    op.execute("DROP TABLE IF EXISTS expense_tracker.recurring_transactions CASCADE")
    op.execute("DROP TYPE IF EXISTS expense_tracker.recurrence_frequency")
